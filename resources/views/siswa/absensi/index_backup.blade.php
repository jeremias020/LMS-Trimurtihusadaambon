...
<div class="container-fluid">
    ...
    @php
        ...
        $hadirCount = optional($breakdown->where('status', 'present')->first())->count ?? 
                     optional($breakdown->where('status', 'hadir')->first())->count ?? 0;
        $izinCount = optional($breakdown->where('status', 'izin')->first())->count ?? 0;
        $sakitCount = optional($breakdown->where('status', 'sick')->first())->count ?? 
                      optional($breakdown->where('status', 'sakit')->first())->count ?? 0;
                                ...
                                @forelse($attendances as $attendance)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @elseif($attendance->status == 'sick' || $attendance->status == 'sakit')
