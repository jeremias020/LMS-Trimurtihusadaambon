...
class AttendanceController extends Controller
{
    ...
    public function index(): View
    {
        $date = request('date', Carbon::today()->format('Y-m-d'));
        $class = request('class', 'all');
        $type = request('type', 'regular'); // 'regular' atau 'praktik'
        ...
        // Filter berdasarkan tipe absensi (regular atau praktik)
        ...
        $statsQuery = Attendance::whereDate('tanggal', $date)
        ...
        $stats = [
            ...
            'hadir' => $statsQuery['hadir'] ?? 0,
            'izin' => $statsQuery['izin'] ?? 0,
            'sakit' => $statsQuery['sakit'] ?? 0,
            'alpha' => $statsQuery['alpha'] ?? 0,
        ];
    ...
    public function create(): View
    {
        ...
        $siswas = User::where('role', 'siswa')
            ->when($selectedClass, function($query, $class) {
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        // ✅ Cek duplikasi
        if ($this->isAttendanceExists($request->siswa_id, $request->tanggal)) {
    ...
    public function bulkStore(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            foreach ($siswas as $siswa) {
                if (!$this->isAttendanceExists($siswa->id, $request->tanggal)) {
                    Attendance::create([
                        'siswa_id' => $siswa->id,
                        'tanggal' => $request->tanggal,
                        'status' => $request->status,
