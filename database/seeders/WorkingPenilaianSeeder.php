...
class WorkingPenilaianSeeder extends Seeder
{
    public function run()
    {
        $this->command->info('Creating working penilaian data...');
        ...
        if (!$practical) {
            ...
            $practicalId = DB::table('practicals')->insertGetId([
                ...
                'durasi' => 120, // 2 hours
                'tools' => 'Stetoskop, Termometer, Alat suntik, Dll',
                'bahan' => 'Kapas betadine, Handscoon, Dll',
                'instruksi' => 'Lakukan praktikum tindakan keperawatan dasar sesuai SOP',
                'is_published' => true,
                'waktu_mulai' => '08:00:00',
                'waktu_selesai' => '10:00:00',
                'skill_level' => 'Menengah',
                'keselamatan' => 'Gunakan APD dengan benar',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        } else {
        ...
        $this->command->info('✅ Working penilaian data created successfully!');
        ...
        $this->command->info('');
        $this->command->info('🎉 Auto assessment is now ready!');
        $this->command->info('📱 Visit: /guru/penilaian/auto');
        $this->command->info('🎯 3 students have practical submissions ready for grading');
        $this->command->info('📝 2 students have assignment submissions ready for grading');
        $this->command->info('🤖 Guru can now use auto assessment with checklist criteria!');
    }
}
